...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        # Question 2
        # determine if the node already exists, using 'has_node' method.
        # if it returns 'True', do not put into the graph
        # if it returns 'False', then put it in the graph

        # express conditional in terms of condensed boolean condition
        # if True, node already exists in the graph
        if self.has_node(node):
            raise ValueError
        # if False, node is not in the graph in which case we add it to the graph
        else:
        # append 'node' to the set self.node
            self.nodes.add(node)
            self.edges[node] = []
    ...
    def add_edge(self, edge):
        ...
        in the graph."""
        # Question 3
        # ensure that the Edge or WeightedEdge does not already exist within the graph
        if not self.has_node(edge.src):
            raise ValueError
        elif not self.has_node(edge.dest):
            raise ValueError
        else:
            edge[edge.src].append(edge)
...
if __name__ == "__main__":
    ...
    b = Node(36)
    print(a)
    digraph_instance = Digraph()
    digraph_instance.add_node(a)
    print(digraph_instance.has_node(a))

    digraph_instance.add_node(b)
    print(digraph_instance.has_node(b))

    print(digraph_instance.get_nodes())

    c = WeightedEdge(a, b, 70, 0)
    digraph_instance.add_edge(c)
    print(digraph_instance.get_edges_for_node(a))